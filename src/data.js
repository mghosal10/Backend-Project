...
//sql queries
let etfSql = "replace into etfdata (ticker, etfname, description, date) values ?";
let holdingsSql = "replace into etfholdings (holdkey, ticker, holdingsname, holdweight, date) values ?";
let sectorsSql = "replace into sectors (sectorkey, ticker, sectorname, sectorweight, date) values ?";
let countriesSql = "replace into geography (geokey, ticker, country, geoweight, date) values ?";
		...
		for(let i=0; i<fundDetails.length ;i++)
		{
			...
			request(fundUrl, (detail_error, detail_response, html) => {
				if(!detail_error && detail_response.statusCode === 200)
				{
					...
					let date = new Date();
					const fundsArr = [fundTicker, fundName, fundDescription, date];
...
function addToTable(arr, sql) {
	...
	con.connect(function(err) {
		...
		con.query(sql,[arr],function(err, result) 
		{
			...
			// if got the result, close connection
			if(result)
			{
				con.end();
			}
